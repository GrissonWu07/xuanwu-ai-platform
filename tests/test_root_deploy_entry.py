from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]


def test_root_compose_exists_and_hosts_platform_stack() -> None:
    compose_file = ROOT / "docker-compose.yml"
    assert compose_file.exists()

    content = compose_file.read_text(encoding="utf-8")
    for service_name in [
        "postgres:",
        "xuanwu-portal:",
        "xuanwu-management-server:",
        "xuanwu-jobs:",
        "xuanwu-device-gateway:",
        "xuanwu-iot-gateway:",
        "mosquitto:",
    ]:
        assert service_name in content


def test_root_compose_uses_root_deploy_mounts() -> None:
    compose_file = ROOT / "docker-compose.yml"
    content = compose_file.read_text(encoding="utf-8")

    assert "./deploy/data/device-gateway:/opt/xuanwu-device-gateway/data" in content
    assert "./deploy/data/pg:/var/lib/postgresql/data" in content
    assert "./deploy/logs/postgres:/var/log/postgresql" in content
    assert "./deploy/logs/mosquitto:/mosquitto/log" in content
    assert "./deploy/logs/portal:/var/log/nginx" in content
    assert "./deploy/logs/device-gateway:/opt/xuanwu-device-gateway/tmp" in content
    assert "./deploy/logs/management-server:/opt/logs" in content
    assert "./deploy/logs/iot-gateway:/opt/logs" in content
    assert "./deploy/logs/jobs:/opt/logs" in content
    assert (
        "./deploy/models/SenseVoiceSmall/model.pt:"
        "/opt/xuanwu-device-gateway/models/SenseVoiceSmall/model.pt"
    ) in content
    assert "XUANWU_PG_DB=" in content
    assert "XUANWU_MGMT_PG_SCHEMA=" in content
    assert "XUANWU_IOT_PG_SCHEMA=" in content


def test_root_setup_script_bootstraps_repo_root() -> None:
    setup_script = (ROOT / "docker-setup.sh").read_text(encoding="utf-8")

    assert "/opt/xuanwu-ai-platform" in setup_script
    assert 'COMPOSE_PATH="$PROJECT_ROOT/docker-compose.yml"' in setup_script
    assert 'DATA_DIR="$PROJECT_ROOT/deploy/data/device-gateway"' in setup_script
    assert 'POSTGRES_DATA_DIR="$PROJECT_ROOT/deploy/data/pg"' in setup_script
    assert 'LOG_ROOT="$PROJECT_ROOT/deploy/logs"' in setup_script
    assert 'MODEL_DIR="$PROJECT_ROOT/deploy/models/SenseVoiceSmall"' in setup_script
    assert "git clone https://github.com/GrissonWu07/xuanwu-ai-platform.git" in setup_script
    assert 'ENV_PATH="$PROJECT_ROOT/.env"' in setup_script


def test_root_compose_keeps_postgres_dependency_chain() -> None:
    content = (ROOT / "docker-compose.yml").read_text(encoding="utf-8")

    assert "depends_on:\n      - postgres" in content
    assert "xuanwu-management-server" in content
    assert "xuanwu-iot-gateway" in content
    assert "      - xuanwu-management-server" in content
    assert "      - mosquitto" in content


def test_root_env_example_exposes_xuanwu_endpoint() -> None:
    env_example = (ROOT / ".env.example").read_text(encoding="utf-8")

    assert "XUANWU_BASE_URL=http://xuanwu-ai:8000" in env_example
    assert "XUANWU_CONTROL_PLANE_SECRET=" in env_example
    assert "XUANWU_PG_DB=" in env_example
    assert "XUANWU_MGMT_PG_SCHEMA=xw_mgmt" in env_example
    assert "XUANWU_IOT_PG_SCHEMA=xw_iot" in env_example


def test_gitignore_covers_root_deploy_runtime_artifacts() -> None:
    gitignore_text = (ROOT / ".gitignore").read_text(encoding="utf-8")

    assert "deploy/data/" in gitignore_text
    assert "deploy/logs/" in gitignore_text
    assert "!deploy/data/device-gateway/" in gitignore_text
    assert "!deploy/data/device-gateway/.gitkeep" in gitignore_text
    assert "!deploy/data/pg/" in gitignore_text
    assert "!deploy/data/pg/.gitkeep" in gitignore_text
    assert "!deploy/logs/postgres/" in gitignore_text
    assert "!deploy/logs/postgres/.gitkeep" in gitignore_text
    assert "!deploy/logs/mosquitto/" in gitignore_text
    assert "!deploy/logs/mosquitto/.gitkeep" in gitignore_text
    assert "!deploy/logs/portal/" in gitignore_text
    assert "!deploy/logs/portal/.gitkeep" in gitignore_text
    assert "!deploy/logs/device-gateway/" in gitignore_text
    assert "!deploy/logs/device-gateway/.gitkeep" in gitignore_text
    assert "!deploy/logs/management-server/" in gitignore_text
    assert "!deploy/logs/management-server/.gitkeep" in gitignore_text
    assert "!deploy/logs/iot-gateway/" in gitignore_text
    assert "!deploy/logs/iot-gateway/.gitkeep" in gitignore_text
    assert "!deploy/logs/jobs/" in gitignore_text
    assert "!deploy/logs/jobs/.gitkeep" in gitignore_text
    assert "deploy/models/SenseVoiceSmall/model.pt" in gitignore_text
