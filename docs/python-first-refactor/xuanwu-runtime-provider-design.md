...
本文档定义 `AtlasClaw` 侧的 `xuanwu-runtime` provider 设计。

该 provider 是 `AtlasClaw` 与 `xuanwu-server` 之间的能力桥，负责把 Agent 的工具调用转成对 `xuanwu-server runtime API` 的远程调用。

## 2. 核心原则

- `AtlasClaw` 负责智能决定
- `xuanwu-server` 负责本地执行
- 所有设备本地高权限动作都通过 provider 调用，不允许进程内直连
- provider 是唯一允许直接访问 `/runtime/v1/*` 的 AtlasClaw 模块

## 3. provider 定位

在 `AtlasClaw` 中，`xuanwu-runtime` 应属于“设备运行时 provider”。
...
- `providers/xuanwu-runtime-provider/`
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `base_url` | string | 是 | `xuanwu-server` runtime API 地址 |
| `runtime_secret` | string | 是 | 服务间认证密钥 |
| `timeout_seconds` | int | 否 | 超时时间 |

## 6. client 设计

建议新增 `XuanwuRuntimeClient`：
...
### 7.1 `xuanwu_get_runtime_context`
...
### 7.3 `xuanwu_speak_text`

用途：

- 在需要即时反馈时播报短文本

### 7.4 `xuanwu_interrupt`
...
在 `xuanwu_execute_local_tool` 之外，建议提供更友好的专用工具：

- `xuanwu_take_photo`
- `xuanwu_call_iot`
- `xuanwu_call_mcp_tool`
...
所有 `xuanwu-runtime` tools 默认从 `ctx.deps.extra["context"]` 中读取：
...
当前 `xuanwu-server` 的 `UnifiedToolHandler` 继续保留，但角色变更为：

- `xuanwu-runtime provider` 背后的本地执行器
...
- 与主对话引擎解耦，不直接依赖 `xuanwu-server` 内部类
