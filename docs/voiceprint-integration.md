...
注意是将`xuanwu-device-gateway-db`下面的`expose`改成`ports`。改完后，需要重新启动。以下是重启mysql的命令：
...
## 第一步 配置接口
打开 `xuanwu-device-gateway/data/.config.yaml` 文件（如果没有需要创建），然后添加/修改以下内容：
