# 视觉模型使用指南
本教程分为两部分：
- 第一部分：单模块运行xuanwu-device-server开启视觉模型
...
## 单模块运行xuanwu-device-server开启视觉模型
...
留意xuanwu-device-server的日志输出，看看有没有报错。
