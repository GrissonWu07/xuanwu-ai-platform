# xuanwu-gateway 合同规范 Spec

## 1. 目的

本文档定义独立模块 `xuanwu-gateway` 的统一合同。
...
2. `XuanWu` 获取聚合视图
3. `XuanWu` 决定调用哪个设备能力
4. `XuanWu` 调用 `xuanwu-gateway`
5. `xuanwu-gateway` 根据网关路由和协议适配器执行动作
6. `xuanwu-gateway` 返回统一执行结果
7. `xuanwu-gateway` 把事件、状态、遥测回流到 `xuanwu-management-server`
...
### 5.2 `xuanwu-gateway -> xuanwu-management-server`
...
### 5.3 `xuanwu-management-server -> xuanwu-gateway`
...
### 6.1 `XuanWu -> xuanwu-gateway`

固定请求头：

- `X-Xuanwu-Gateway-Secret`
- `X-Request-Id`
- `X-Trace-Id`

### 6.2 `xuanwu-gateway -> xuanwu-management-server`
...
`xuanwu-gateway` 必须按设备类型和协议分目录组织。
...
`xuanwu-gateway` 必须成为独立的协议与能力适配平台。
