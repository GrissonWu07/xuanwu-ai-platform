# xuanwu-iot-gateway 模块蓝图

## 1. 目的

本文档定义 `xuanwu-iot-gateway` 作为独立协议与能力适配模块的实现蓝图。

## 2. 模块定位

`xuanwu-iot-gateway` 是独立部署、独立扩展、独立维护的南向适配层。
...
- 会话型设备主要仍由 `xuanwu-device-gateway` 负责接入
- `xuanwu-iot-gateway` 只承担补充网关适配职责
...
`xuanwu-iot-gateway` 必须独立成为平台的南向协议与能力适配层，而不是散落在各个服务中的临时逻辑集合。
