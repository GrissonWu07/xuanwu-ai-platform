# xuanwu-iot-gateway Adapter Completion Spec

## Purpose

This document defines the missing implementation work required to move `xuanwu-iot-gateway` from a registry-and-skeleton foundation into a usable IoT and industrial protocol execution layer.
...
`xuanwu-iot-gateway` must support real protocol-backed execution for the first production-ready adapter set, while preserving the existing unified command contract.
...
Conversation devices remain primarily owned by `xuanwu-device-gateway`, so these gateway adapter directories can stay lower priority for now.
...
It must also document that an MQTT broker is external infrastructure and not an in-process replacement inside `xuanwu-iot-gateway`.
...
- `xuanwu-iot-gateway` is no longer accurately described as "skeleton adapters"
