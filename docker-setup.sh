...
check_installed() {
    # 检查目录是否存在且非空
    if [ -d "/opt/xuanwu-device-server/" ] && [ "$(ls -A /opt/xuanwu-device-server/)" ]; then
        DIR_CHECK=1
    else
        DIR_CHECK=0
    fi
    ...
    # 检查容器是否存在
    if docker inspect xuanwu-device-server > /dev/null 2>&1; then
        CONTAINER_CHECK=1
    else
        CONTAINER_CHECK=0
    fi
...
if check_installed; then
    if whiptail --title "已安装检测" --yesno "检测到小智服务端已安装，是否进行升级？" 10 60; then
        ...
        docker compose -f /opt/xuanwu-device-server/docker-compose_all.yml down
        
        # 停止并删除特定容器（考虑容器可能不存在的情况）
        containers=(
            "xuanwu-device-server"
            "xuanwu-device-server-web"
        ...
        # 备份原有配置文件
        mkdir -p /opt/xuanwu-device-server/backup/
        if [ -f /opt/xuanwu-device-server/data/.config.yaml ]; then
            cp /opt/xuanwu-device-server/data/.config.yaml /opt/xuanwu-device-server/backup/.config.yaml
            echo "已备份原有配置文件到 /opt/xuanwu-device-server/backup/.config.yaml"
        fi
        
        # 下载最新版配置文件
        check_and_download "/opt/xuanwu-device-server/docker-compose_all.yml" "https://ghfast.top/https://raw.githubusercontent.com/GrissonWu07/ai-assist-deviceserver/refs/heads/main/main/xuanwu-device-server/docker-compose_all.yml"
        check_and_download "/opt/xuanwu-device-server/data/.config.yaml" "https://ghfast.top/https://raw.githubusercontent.com/GrissonWu07/ai-assist-deviceserver/refs/heads/main/main/xuanwu-device-server/config_from_api.yaml"
        
        # 启动Docker服务
        echo "开始启动最新版本服务..."
        # 升级完成后标记，跳过后续下载步骤
        UPGRADE_COMPLETED=1
        docker compose -f /opt/xuanwu-device-server/docker-compose_all.yml up -d
    else
...
echo "开始创建安装目录..."
# 检查并创建数据目录
if [ ! -d /opt/xuanwu-device-server/data ]; then
    mkdir -p /opt/xuanwu-device-server/data
    echo "已创建数据目录: /opt/xuanwu-device-server/data"
else
    echo "目录device-server/data已存在，跳过创建"
fi

# 检查并创建模型目录
if [ ! -d /opt/xuanwu-device-server/models/SenseVoiceSmall ]; then
    mkdir -p /opt/xuanwu-device-server/models/SenseVoiceSmall
    echo "已创建模型目录: /opt/xuanwu-device-server/models/SenseVoiceSmall"
else
    echo "目录device-server/models/SenseVoiceSmall已存在，跳过创建"
fi

echo "------------------------------------------------------------"
echo "开始下载语音识别模型"
# 下载模型文件
MODEL_PATH="/opt/xuanwu-device-server/models/SenseVoiceSmall/model.pt"
...
while true; do
    ...
    if docker logs xuanwu-device-server-web 2>&1 | grep -q "Started AdminApplication in"; then
        break
    fi
    sleep 1
done
