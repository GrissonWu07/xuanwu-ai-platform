...
check_installed() {
    # 检查目录是否存在且非空
    if [ -d "/opt/device-server/" ] && [ "$(ls -A /opt/device-server/)" ]; then
        DIR_CHECK=1
    else
        DIR_CHECK=0
    fi
...
if check_installed; then
    if whiptail --title "已安装检测" --yesno "检测到小智服务端已安装，是否进行升级？" 10 60; then
        ...
        docker compose -f /opt/device-server/docker-compose_all.yml down
        ...
        # 备份原有配置文件
        mkdir -p /opt/device-server/backup/
        if [ -f /opt/device-server/data/.config.yaml ]; then
            cp /opt/device-server/data/.config.yaml /opt/device-server/backup/.config.yaml
            echo "已备份原有配置文件到 /opt/device-server/backup/.config.yaml"
        fi
        ...
        # 升级完成后标记，跳过后续下载步骤
        UPGRADE_COMPLETED=1
        docker compose -f /opt/device-server/docker-compose_all.yml up -d
    else
...
echo "开始创建安装目录..."
# 检查并创建数据目录
if [ ! -d /opt/device-server/data ]; then
    mkdir -p /opt/device-server/data
    echo "已创建数据目录: /opt/device-server/data"
else
    echo "目录device-server/data已存在，跳过创建"
fi

# 检查并创建模型目录
if [ ! -d /opt/device-server/models/SenseVoiceSmall ]; then
    mkdir -p /opt/device-server/models/SenseVoiceSmall
    echo "已创建模型目录: /opt/device-server/models/SenseVoiceSmall"
else
...
echo "开始下载语音识别模型"
# 下载模型文件
MODEL_PATH="/opt/device-server/models/SenseVoiceSmall/model.pt"
