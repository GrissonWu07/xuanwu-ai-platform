...
from core.utils.modules_initialize import initialize_modules, initialize_dialogue_engine
...
class WebSocketServer:
    def __init__(self, config: dict):
        ...
        self.dialogue_engine = initialize_dialogue_engine(self.config)
    ...
    async def update_config(self) -> bool:
        ...
        try:
            async with self.config_lock:
                if not should_use_dynamic_server_config(self.config):
                    self.logger.bind(tag=TAG).warning("当前未启用动态控制面配置刷新")
                    return False
                # 重新获取配置（使用异步版本）
