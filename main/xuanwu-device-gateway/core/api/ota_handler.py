...
class OTAHandler(BaseHandler):
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # 如果配置了非空字符串
                ...
                return_json["mqtt"] = {
                    ...
                    "username": username,
                    "password": password,
                    "publish_topic": "xuanwu-device-gateway",
            ...
            try:
                ...
                for ver, fname in candidates:
                    if _is_higher_version(ver, device_version):
                        ...
                        # Replace the path from "/mcp/vision/explain" to "/xuanwu/ota/download/{fname}"
                        chosen_url = vision_url.replace(
                            "/mcp/vision/explain", f"/xuanwu/ota/download/{fname}"
                        )
                        break
    ...
    async def handle_download(self, request):
        """
        下载固件接口
        URL: /xuanwu/ota/download/{filename}
